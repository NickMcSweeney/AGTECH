...
# include the local resources directory to the path to give acess to utils
...
class VControl:
    ...
    def getVel(self, set_point):
        ...
        if(set_point == 0 and abs(self.vel) >= 0.05):
            ...
            diff = abs(self.vel)
            inc = abs((self.speed+abs(self.vel))/25) if self.vel < 0 else (-1)*abs((self.speed+abs(self.vel))/25)
        else: 
...
if __name__ == '__main__':
    ...
    pub_drive = rospy.Publisher('/'+resource_name(host_name)+'/cmd_vel', Twist, queue_size=1)
    pub_gripper = rospy.Publisher('/'+resource_name(host_name)+'/set_gripper_pos', Float64, queue_size=1)
    pub_arm = rospy.Publisher('/'+resource_name(host_name)+'/set_arm_pos', Float64, queue_size=1)
        ...
        while not rospy.is_shutdown():
            ...
            if key in moveBindings.keys(): # if key is a movement key
                ...
                if (status == 14):
                    print(msg)
                status = (status + 1) % 15
            elif key in posBindings.keys(): # is key is for setting the arm or gripper position
                arm = arm + posBindings[key][0] if 0 <= arm + posBindings[key][0] <= 100 else arm 
                gripper = gripper + posBindings[key][1] if 0 <= gripper + posBindings[key][1] <= 100 else gripper
            ...
            vel_msg.angular.z = th*turn; # ...rotational velocity
